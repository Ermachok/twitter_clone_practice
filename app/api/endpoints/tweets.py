...
async def get_user_feed(
    ...
    tweets_result = await db.execute(
        select(Tweet).where(Tweet.author_id.in_(following_ids)).order_by(Tweet.id.desc())
    )
    ...
    for tweet in tweets:
        ...
        likes_data = []
        for like in likes:
            user_like_result = await db.execute(select(User.name).where(User.id == like.user_id))
...
@router.get("/feed")
async def get_feed(api_key: str = Header(...), db: AsyncSession = Depends(get_db)):
    user_result = await db.execute(select(User).where(User.name == api_key))
    user = user_result.scalars().first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    tweets = tweets_result.scalars().unique().all()

    return {
        "result": True,
        "tweets": [{"id": t.id, "content": t.content, "author_id": t.author_id} for t in tweets],
    }
